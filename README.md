...
- **Операционная система**: Linux с systemd (например, Ubuntu 20.04+, Debian 10+, CentOS 8+).
- **Утилиты**:
  - `bash`: Для выполнения скрипта (обычно предустановлен).
  - `pgrep`: Для поиска PID процесса (обычно предустановлен).
  - `curl`: Для HTTPS-запросов. Установите, если отсутствует:
    ```bash
    sudo apt install curl  # Для Debian/Ubuntu
    sudo yum install curl  # Для CentOS/RHEL
    ```
- **Права доступа**: Доступ с `sudo` для размещения файлов и настройки systemd.
- **Процесс `test`**: Процесс с именем `test` должен существовать в системе. Если имя другое, измените строку `CURRENT_PID=$(pgrep -o test)` в скрипте.
